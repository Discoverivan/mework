...
- Store production/release credentials only through the OS keyring. In `debug_assertions`, provider credentials may be supplied transiently through `MEWORK_DEV_JIRA_PAT` and `MEWORK_DEV_BITBUCKET_PAT`; never commit them, persist them in SQLite, print them, or return them to the renderer. SQLite, logs, diagnostics exports, prompts, frontend bundles, and Git history must not contain tokens or raw authorization headers.
